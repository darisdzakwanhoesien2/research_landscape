...
st.caption("Keyword mapping + manual overrides + audit logging")

# =========================================================
# SESSION STATE INIT  ✅ FIXES TABLE RESET
# =========================================================
...
def batch_classify(records, mapping):
    ...
    for r in records:
        ...
        if rid in overrides:
            ...
            confidence = overrides[rid].get("confidence", 100)
            matched_keywords = "MANUAL_OVERRIDE"
        else:
            final_category, matched_keywords, confidence = classify_text(
                r["text"], mapping
            )
...
if run_btn and records:
    ...
    # Store results in session (prevents table reset)
...
if st.session_state.edited_df is not None:
    ...
    st.subheader("✍️ Manual Override (Persistent Editor)")
